...
class BaseClient(object):
    ...
    def _login_decorator(self, func, *args, **kwargs):
        ...
        try:
            ...
            #  LOGGERinfo("re-logging in due to LOGIN_TIMEOUT gone")
    ...
    def get_chart_last_request(self, symbol, period, start):
        ...
        #  self.LOGGER.info(f"CMD: get chart last request for {symbol} of period" f" {period} from {start}...")
    ...
    def get_chart_range_request(self, symbol, period, start, end, ticks):
        ...
        #  self.LOGGER.info(f"CMD: get chart range request for {symbol} of " f"{period} from {start} to {end} with ticks of " f"{ticks}...")
        return self._send_command_with_check(data)
    ...
    def get_profit_calculation(self, symbol, mode, volume, op_price, cl_price):
        ...
        data = _get_data("getProfitCalculation", closePrice=cl_price,
                         ...
                         volume=volume)
        #  self.LOGGER.info(f"CMD: get profit calculation for {symbol} of " f"{volume} from {op_price} to {cl_price} in mode " f"{mode}...")
        return self._send_command_with_check(data)
    ...
    def get_tick_prices(self, symbols, start, level=0):
        ...
        #  self.LOGGER.info(f"CMD: get tick prices of {symbols} from {start} " f"with level {level}...")
        return self._send_command_with_check(data)
    ...
    def get_trade_records(self, trade_position_list):
        ...
        #  self.LOGGER.info(f"CMD: get trade records of len " f"{len(trade_position_list)}...")
        return self._send_command_with_check(data)
    ...
    def get_trading_hours(self, trade_position_list):
        ...
        #  self.LOGGER.info(f"CMD: get trading hours of len " f"{len(trade_position_list)}...")
...
class Transaction(object):
    def __init__(self, trans_dict):
        ...
        self.open_price = trans_dict['open_price']
        self.sl = trans_dict['sl']
        self.tp = trans_dict['tp']

        #  LOGGERdebug(f"Transaction {self.order_id} inited")
...
class Client(BaseClient):
    ...
    def check_if_market_open(self, list_of_symbols):
        ...
        for symbol in response:
            ...
            elif today_values[0]['fromT'] <= actual_tmsp <= today_values[0]['toT']:
    ...
    def get_lastn_candle_history(self, symbol, timeframe_in_seconds, number):
        ...
        #  LOGGERdebug(f"sym: {symbol}, tmf: {timeframe_in_seconds}," f" {time.time() - sec_prior}")
        res = {'rateInfos': []}
        ...
        #  LOGGERdebug(candle_history)
        return candle_history

    def get_range_candle_history(self, symbol, period, start, end, ticks):
        """get last n candles of timeframe"""
        #  LOGGERdebug(f"sym: {symbol}, tmf: {timeframe_in_seconds}," f" {time.time() - sec_prior}")
        res = self.get_chart_range_request(symbol, period//60, start, end, ticks)
        candle_history = []
        for candle in res['rateInfos']:
            _pr = candle['open']
            op_pr = _pr / 10 ** res['digits']
    ...
    def open_trade(self, mode, symbol, volume):
        ...
        mode_name = mode.name
        mode = mode.value
        #  self.LOGGER.debug(f"opening trade of {symbol} of {volume} with " f"{mode_name}")
